function joint_points = discretize_posture(joint_lengths)
...
while j <= K % Loop through every joint. Could write this recursively
    ...
    while get_distance(base_pt, [x_cross, y_cross]) <= length_j
       ...
       error_pt = root_mean_square_error(base_pt, end_pt);
       if(error_pt < error)
           end_pt_best = end_pt; 
       end
